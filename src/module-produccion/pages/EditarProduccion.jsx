...
import {
  addProduccionProvider,
  getProduccionProvider,
  updateProduccionProvider,
} from "../../providers/produccion/providerProduccion";
...
export const EditarProduccion = () => {
  ...
  const getProduccion = async () => {
    const { ok, data } = await getProduccionProvider(id);
    if (ok) {
  ...
  const onSubmit = async (values, e) => {
    ...
    const { data, ok, message } = await updateProduccionProvider(values, id);
    if (ok) {
      setOpen(true);
      setError(false);
      setTimeout(() => {
